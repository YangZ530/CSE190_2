...
void Cube::zRotate(float radians){
	Matrix4 rotate;
	rotate.makeRotateZ(radians);
	toWorld = rotate*toWorld;
}

void Cube::xMove(float d){
	Matrix4 xm;
	xm.makeTranslate(d,0,0);
	toWorld = xm*toWorld;
}

void Cube::yMove(float d){
	Matrix4 ym;
	ym.makeTranslate(0, d, 0);
	toWorld = ym*toWorld;
}

void Cube::zMove(float d){
	Matrix4 zm;
	zm.makeTranslate(0, 0, d);
	toWorld = zm*toWorld;
}
